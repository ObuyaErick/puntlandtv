...
void main() {
  ...
  group('metadata does not age the story', () {
    test('flipping breaking news leaves every translation where it was',
        () async {
      final editor = await open('a-rains');
      final stamps = {
  ...
  group('a failed write', () {
    test('keeps the editor\'s text rather than reverting to the server',
        () async {
      final editor = ArticleEditor(
...
class _AlwaysFails implements PuntlandAdminApi {
  ...
  dynamic noSuchMethod(Invocation invocation) =>
      Future<Never>.error(const Failure(kind: FailureKind.offline, code: 'OFFLINE'));
}
