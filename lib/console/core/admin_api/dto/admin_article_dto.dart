...
class AdminArticleDto {
  ...
  /// The URL segment the story is published at. Generated by the backend when
  /// the draft is created and stable from then on — a slug that followed the
  /// headline would break every link the moment a sub-editor tightened it.
  final String slug;
  ...
  /// The hero image's asset id — the field the console actually writes.
  ///
  /// [imageUrl] is what the library resolved that id to, and is not the
  /// console's to set. Attaching by id is what lets the backend refuse an
  /// asset that has not finished ingesting, and what makes the media library's
  /// "in use" rule answerable: a URL says nothing about which row a deletion
  /// would break.
  ...
  Map<String, dynamic> toJson() => {
    ...
    // Read back by [fromJson] and previously dropped here, which made the
    // fixture store lose which language a story was written in the moment it
    // was serialised — every article came back claiming Somali was its source.
  ...
  /// Every field, including the nullable ones.
  ///
  /// The nullable fields take [clearScheduledFor], [clearPublishedAt] and
  /// [clearImage] rather than reading `null` as "clear", because `null` here
  /// already means "leave alone" — and the two are not interchangeable when
  /// un-scheduling a story is a thing an editor does.
  ///
  /// [updatedAt] is a parameter rather than a stamp of `DateTime.now()`. It
  /// used to be the latter, which made this method unusable for anything that
  /// must *not* touch freshness: flipping the breaking-news switch aged the
  /// article past its own translations, and the console reported a translation
  /// as behind because somebody had toggled a flag.
  AdminArticleDto copyWith({
    ...
    imageAlt: clearImage ? null : imageAlt ?? this.imageAlt,
  );

  /// The article as it stands with one language's text replaced.
  ///
  /// The freshness model lives here rather than in each caller: saving a
  /// translation moves that translation's clock *and* the article's, and
  /// nothing else's. Stamping every locale would clear the stale flag on a
  /// language nobody had touched.
  AdminArticleDto withTranslation(
    String locale,
